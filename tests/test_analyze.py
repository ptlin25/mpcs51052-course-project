...
PLAYER_POSITIONS = {
    ...
    12: Position.GKP, 13: Position.DEF, 14: Position.MID, 15: Position.FWD,
}

# starters each score 5, bench players each score 0
BASE_POINTS = {i: 5 for i in ALL_STARTERS} | {i: 0 for i in {GKP_BENCH, DEF_BENCH, MID_BENCH, FWD_BENCH}}
...
def make_gameweek(
    ...
    picks = []
    for i in range(1, 16):
        player = Player(id=i, name=f"Player{i}", position=PLAYER_POSITIONS[i], history=[0] * (round - 1) + [points[i]])
...
class TestAnalyzeGameweek:
    ...
    @pytest.mark.parametrize(
        ...
        [
            pytest.param(
                ...
                {1:10, 2:10, 3:10, 4:10, 5:10, 6:10, 7:10, 8:10, 9:10, 10:10, 11:10, 12:0, 13:0, 14:0, 15:0},
                ALL_STARTERS,
                id="all_starters_optimal",
            ),
            pytest.param(
                # bench GKP (12) outscores starting GKP (1)
                {1:1, 2:6, 3:4, 4:5, 5:8, 6:9, 7:5, 8:7, 9:6, 10:5, 11:5, 12:3, 13:0, 14:0, 15:0},
            ...
            pytest.param(
                ...
                {1:5, 2:8, 3:7, 4:6, 5:1, 6:8, 7:7, 8:6, 9:5, 10:8, 11:1, 12:0, 13:1, 14:5, 15:9},
                ALL_STARTERS - {5, 11} | {MID_BENCH, FWD_BENCH},
                id="optimal_starters_changes_formation",
            ),
            pytest.param(
                # min 3 DEF still required, flex goes to MID/FWD
                {1:5, 2:1, 3:1, 4:1, 5:0, 6:5, 7:5, 8:5, 9:5, 10:10, 11:10, 12:0, 13:0, 14:5, 15:4},
                ALL_STARTERS - {5} | {MID_BENCH},
                id="min_3_defenders",
            ),
        ],
    )
    def test_optimal_starters_selects_highest_scoring_valid_11(self, points, expected_ids):
    ...
    def test_triple_captain_has_correct_captain(self):
        ...
        gameweek: GameweekAnalysis = self.analyze(BASE_POINTS, captain_multiplier=3, active_chip=Chip.TC)
...
class TestAnalyzeSeason:
    ...
    def test_actual_points_no_chips_sums_starters_and_captain(self):
        ...
        """
        season: SeasonAnalysis = self.analyze({
            1: make_gameweek(BASE_POINTS, round=1, captain_multiplier=3, active_chip=Chip.TC),
        })
    ...
    def test_optimal_points_no_chips_uses_best_selection(self):
        ...
        """
        season: SeasonAnalysis = self.analyze({
            1: make_gameweek({**BASE_POINTS, 15: 20}, round=1, captain_multiplier=3, active_chip=Chip.TC),
        })
    ...
    def test_actual_chip_usage_records_chips_as_played(self):
        ...
        assert season.actual_chip_usage == {1: ChipUsage(Chip.FH, 0), 2: ChipUsage(Chip.BB, 0)}

    def test_actual_tc_chip_bonus(self):
        """Test actual TC bonus is equal to the captain's points."""
        season: SeasonAnalysis = self.analyze({
            1: make_gameweek({**BASE_POINTS, CAPTAIN: 10}, round=1, active_chip=Chip.TC),
        })
    ...
    def test_actual_bb_chip_bonus(self):
        ...
        season: SeasonAnalysis = self.analyze({
            1: make_gameweek({**BASE_POINTS, 12: 2, 15: 4}, round=1, active_chip=Chip.BB),
        })
    ...
    def test_optimal_chip_usage_reassigns_tc_and_bb_to_best_gameweeks(self):
        ...
        season: SeasonAnalysis = self.analyze({
            ...
            2: make_gameweek({**BASE_POINTS, CAPTAIN: 10}, round=2, active_chip=Chip.BB),
        })
        assert season.optimal_chip_usage == {1: ChipUsage(Chip.BB, 1), 2: ChipUsage(Chip.TC, 10)}
